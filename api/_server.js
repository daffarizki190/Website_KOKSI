...
function formatWIBTime(date = /* @__PURE__ */ new Date()) {
    ...
    switch (callbackData) {
      ...
      case "cmd_pesanan": {
        ...
        const activeOrders = await db.select({
          ...
          userName: users.nama,
          userPt: users.pt
        }).from(orders).leftJoin(users, eq(orders.userId, users.id)).where(sql`${orders.status} IN ('Proses', 'Sedang Menyiapkan', 'Menunggu Konfirmasi')`).orderBy(desc(orders.id)).limit(5);
...
Semua transaksi dalam status selesai atau siap.`);
          ...
          await sendTelegramMessage(chatId, `\u{1F4E6} *DAFTAR PESANAN AKTIF (5 TERBARU):*
...
Contoh: \`/selesai 1024\``);
        ...
        const updated = await db.update(orders).set({ status: "Selesai", keterangan: `Diselesaikan via Telegram oleh ${senderName} pada ${formatWIBTime()}` }).where(eq(orders.id, orderId)).returning();
        if (updated.length > 0) {
...
Contoh: \`/batal 1024 Stok kosong\``);
        ...
        const updated = await db.update(orders).set({ status: "Dibatalkan", keterangan: `Dibatalkan via Telegram: ${alasan}` }).where(eq(orders.id, orderId)).returning();
        if (updated.length > 0) {
          try {
            const oItems = await db.select().from(orderItems).where(eq(orderItems.orderId, orderId));
            for (const item of oItems) {
          ...
          await sendTelegramMessage(chatId, `\u{1F6AB} *Pesanan #${orderId} Berhasil Dibatalkan* dan stok barang telah dikembalikan.`);
        } else {
          await sendTelegramMessage(chatId, `\u274C Pesanan #${orderId} tidak ditemukan.`);
        }
        break;
